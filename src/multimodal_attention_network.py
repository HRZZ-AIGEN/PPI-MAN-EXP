from abc import ABC
import torch
from torch.nn import Linear, BatchNorm1d, Dropout1d
...
class MMAtt(torch.nn.Module, ABC):
    def __init__(
            self, mat_depth = 8, mat_heads = 8,
            ppi_depth = 3, 
            dim_in_gat = 7, gat_model_dim = 64, gat_dim_out = 128,
            add_self_loops = True):
    ...
    def forward(self, x, adj_mat, dist_mat, mask, 
                x_ppi, ppi_edge_index, ppi_batch, att_weights=True):
        ...
        x_ppi, att_ppi= self.GAT.forward(x_ppi, ppi_edge_index, ppi_batch, att_weights=att_weights)
...
class Attention(nn.Module, ABC):
    ...
    def forward(self, x, mask=None, adjacency_mat=None, distance_mat=None):
        ...
        assert dist_kernel_fn in DISTANCE_KERNELS, f'distance kernel function needs to be one of {DISTANCE_KERNELS.keys()}'
        dist_kernel_config = DISTANCE_KERNELS[dist_kernel_fn]
...
class GAT(torch.nn.Module, ABC):
    def __init__(self,
        ...
        self.gat_conv_1 = GATConv(dim_in, model_dim, heads=4, add_self_loops=add_self_loops)
        self.lin_1 = Linear(dim_in, model_dim * 4)  # Skip connections

        self.gat_conv_2 = GATConv(model_dim * 4, model_dim, 
                                  heads=4, add_self_loops=add_self_loops)
        self.lin_2 = Linear(model_dim * 4, model_dim * 4)

        self.gat_conv_3 = GATConv(model_dim * 4, dim_out, heads=6, 
                                  add_self_loops=add_self_loops, concat=False)
        self.lin_3 = Linear(model_dim * 4, dim_out)
        self.batch_norm = BatchNorm1d(dim_out)

    def forward(self, x_ppi, ppi_edge_index, ppi_batch, att_weights=True):
        ...
        x_ppi, att_ppi = self.gat_conv_3(x_ppi, ppi_edge_index, return_attention_weights=att_weights)
...
class GATv2(torch.nn.Module, ABC):
    def __init__(self,
                 dim_in=7,
                 model_dim=64,
                 dim_out=128, 
        ...
        # Concatination is the default action of GATConv
        torch.use_deterministic_algorithms(False)
        self.gat_conv_1 = GATv2Conv(dim_in, model_dim, heads=4, add_self_loops=add_self_loops) 
        self.lin_1 = Linear(dim_in, model_dim * 4)  # Skip connections
        self.batch_norm_1 = BatchNorm1d(model_dim * 4)
        self.dropout = Dropout1d(0.4)
        self.gat_conv_2 = GATv2Conv(model_dim * 4, model_dim, heads=4, add_self_loops=add_self_loops)
        self.lin_2 = Linear(model_dim * 4, model_dim * 4)
        self.batch_norm_2 = BatchNorm1d(model_dim * 4)
        self.gat_conv_3 = GATv2Conv(model_dim * 4, dim_out, heads=6, add_self_loops=add_self_loops, concat=False)
        self.lin_3 = Linear(model_dim * 4, dim_out)
        self.batch_norm_3 = BatchNorm1d(dim_out)

    def forward(self, x_ppi, ppi_edge_index, ppi_batch, att_weights=True):
        # torch.cuda.empty_cache()
        x_ppi = F.relu(self.gat_conv_1(x_ppi, ppi_edge_index) + self.lin_1(x_ppi))
        x_ppi = self.batch_norm_1(x_ppi)
        x_ppi = self.dropout(x_ppi)
        x_ppi = F.relu(self.gat_conv_2(x_ppi, ppi_edge_index) + self.lin_2(x_ppi))
        x_ppi = self.batch_norm_2(x_ppi)
        x_res_3 = self.lin_3(x_ppi)  # residual
        x_ppi, att_ppi = self.gat_conv_3(x_ppi, ppi_edge_index, return_attention_weights=att_weights)
        # torch.use_deterministic_algorithms(True)
        x_ppi = x_ppi + x_res_3  # add last residual
        x_ppi = self.batch_norm_3(F.relu(x_ppi))
        x_ppi = global_max_pool(x_ppi, ppi_batch)
        return x_ppi, att_ppi
...
class GAT1(torch.nn.Module, ABC):
    def __init__(self, dim_in=7, model_dim=64, dim_out=128,
                 add_self_loops=False):
        super(GAT1, self).__init__()
        super().__init__()
        self.gat_conv_1 = GATv2Conv(dim_in, dim_out, heads=4, add_self_loops=add_self_loops, concat=False)
    ...
    def forward(self, x_ppi, ppi_edge_index, ppi_batch, att_weights=True):
        ...
        x_ppi, att_ppi = self.gat_conv_1(x_ppi, ppi_edge_index, return_attention_weights=att_weights)
...
class GAT2(torch.nn.Module, ABC):
    def __init__(self, 
        ...
        super().__init__()
        self.gat_conv_1 = GATv2Conv(dim_in, model_dim, 
                                  heads=4, add_self_loops=add_self_loops)
        self.lin_1 = Linear(dim_in, model_dim * 4)  # Skip connections

        self.gat_conv_2 = GATv2Conv(model_dim * 4, dim_out, heads=4, 
                                  add_self_loops=add_self_loops, 
                                  concat=False)
    ...
    def forward(self, x_ppi, ppi_edge_index, ppi_batch, att_weights=True):
        ...
        x_ppi, att_ppi = self.gat_conv_2(x_ppi, ppi_edge_index, return_attention_weights=att_weights)
